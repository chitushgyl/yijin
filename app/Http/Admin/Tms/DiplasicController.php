...
class DiplasicController extends CommonController{
    ...
    public function diplasicPage(Request $request){
        ...
        $select=['self_id','car_id','car_number','production_date','input_date','service','tips','service_now','service_plan','next_service_plan','create_user_name','create_time','group_code','use_flag','img'];
...
//        $self_id='car_202012242220439016797353';
        ...
        $old_info = TmsDiplasic::whereIn('self_id',explode(',',$self_id))->select('use_flag','self_id','delete_flag','group_code')->get();
        $data['delete_flag']='N';
        $data['update_time']=$now_time;
//        dd($old_info);
        $id=TmsDiplasic::whereIn('self_id',explode(',',$self_id))->update($data);
        if ($id){
            $msg['code']=200;
            $msg['msg']="数据拉取成功";
        }else{
