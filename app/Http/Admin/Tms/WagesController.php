...
//dd($button_info);
        ...
        foreach($data['items'] as $k => $v){
            $v->company_fine = UserReward::where('event_time','>=',$start_time)->where('event_time','<',$end_time)->where('driver_id',$v->self_id)->sum('company_fine');
            ...
            $v->reward_price = userExamine::where('create_time','>=',$start_time)->where('create_time','<',$end_time)->where('usre_id',$v->self_id)->sum('reward_price');
        }
