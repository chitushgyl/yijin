...
//        $self_id='car_202012242220439016797353';
    ...
    public function tryeDelFlag(Request $request,Status $status){
        ...
        $data['delete_flag']='N';
        $data['update_time']=$now_time;
        $id = TmsTrye::where('self_id',$self_id)->update($data);
...
//        dd($data['items']->toArray());
        ...
        foreach ($data['items'] as $k=>$v) {
            ...
            foreach ($v->tmsTryeChange as $kk=>$vv) {
                $v->different_count = TmsTryeChange::where('sku_id',$v->self_id)->where('inout_time','<',$start_time)->where('type','different')->sum('different');
                if ($vv->type == 'preentry'){
...
//        dd($msg);
    ...
    public function historyPage(Request $request){
        ...
        $select=['self_id','model','initial_num','change_num','now_num','create_time','now_num','different','trye_name','sku_id','trye_sku_id','group_name','group_code','use_flag','delete_flag','order_id','library_sige_id','price','inout_time','create_time','type'];
