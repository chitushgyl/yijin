...
class PushController extends CommonController{
    ...
    /***    推送管理分页数据      /message/push/pushPage
     *      前端传递必须参数：
     *      前端传递非必须参数：
     */
    ...
    /***    创建后台推送                      /message/push/createPush
     *      前端传递必须参数：
     *      前端传递非必须参数：
     */
    public function createPush(Request $request){
        ...
        if ($validator->passes()) {
            ...
            $select=['self_id','wx_template_id','template_title','template_info'];
            $data['items']=WxMessage::where($where)->orderBy('create_time','desc')->select($select)->get();
            foreach ($data['items'] as $k => $v){
                $v->template_info = json_decode($v->template_info);
            }

            /** 拿去固定的推送人员，需要选择的学校
             *  老师的数据，照管，司机的数据，学生家长的数据，如果产生推送，可能就只能一条一条记录了，不然记录不下来
             */
            $rule=['care','drive','teacher'];
